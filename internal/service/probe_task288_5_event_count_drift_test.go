...
func TestDeclareSplitIncrementsBatchEventCount(t *testing.T) {
	app := newProbeApp(t)
	ctx := context.Background()
	batch, err := app.CreateBatch(ctx, "split", 800, 600)
	if err != nil {
		t.Fatal(err)
	}
	importFrame(t, app, batch.ID, 1)
	parent := importObs(t, app, batch.ID, 1, "P", 100, 100, 10, 1000)
	child1 := importObs(t, app, batch.ID, 1, "C1", 120, 100, 7, 400)
	child2 := importObs(t, app, batch.ID, 1, "C2", 140, 100, 7, 400)
	if _, err := app.Verdict.DeclareSplit(ctx, batch.ID, []*model.Observation{parent}, []*model.Observation{child1, child2}, 0.05, "probe"); err != nil {
		t.Fatal(err)
	}
